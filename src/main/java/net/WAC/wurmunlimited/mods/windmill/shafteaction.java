...
public class shafteaction implements WurmServerMod, ItemTypes, MiscConstants, ModAction, BehaviourProvider, ActionPerformer {
	...
	public List<ActionEntry> getBehavioursFor(Creature performer, Item target) {
		...
		if (performer instanceof Player) {
			if (target.getTemplateId() == net.WAC.wurmunlimited.mods.windmill.windmill.sawmilltemplateid || target.getTemplateId() == net.WAC.wurmunlimited.mods.windmill.windmill.windmilltemplateid)
	...
	public boolean action(Action act, Creature performer, Item target, short action, float counter) {
		...
		if (target.getTemplateId() == net.WAC.wurmunlimited.mods.windmill.windmill.sawmilltemplateid) {
			...
			if (act.currentSecond() % tickTimes == 0) {
				...
				return net.WAC.wurmunlimited.mods.windmill.windmill.windmillItemCreate(performer, target, ItemList.shaft, ItemList.log, 1500, 20, 1000, SoundNames.CARPENTRY_SAW_SND);
			}
